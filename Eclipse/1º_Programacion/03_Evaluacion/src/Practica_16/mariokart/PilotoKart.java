...
public class PilotoKart implements Comparable<PilotoKart> {
	...
	@Override
	public int compareTo(PilotoKart p) {
		if(this.caracteristicas[VELOCIDAD] < p.caracteristicas[VELOCIDAD]) {
			return -1;
		} else if(this.caracteristicas[VELOCIDAD] == p.caracteristicas[VELOCIDAD]) {
			return p.getPiloto().compareTo(this.getPiloto());
		} else {
			return 1;
		}
	}
